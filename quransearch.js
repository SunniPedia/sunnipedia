...
// search box toggle (optional, এখানে সবসময় দেখানো থাকবে)
        ...
        fetch(`surah/${sura.author}.json`)
          ...
          .then(ayahs => {
            ayahs.forEach((ayah, idx) => {
              // ayah কে sura এবং ayah নম্বর attach করে রাখবো
              allAyahs.push({
                ...ayah,
                suraName: sura.name || '',
                ayahNumber: idx + 1,
            ...
            if (loadedCount === suraList.length) {
              ...
              renderAyahs(''); // প্রথমে খালি সার্চ দিয়ে সব দেখাবে
            }
          })
          .catch(err => {
...
function renderAyahs(query) {
  ...
  const filteredAyahs = allAyahs.filter(ayah => {
    return ayah.names?.toLowerCase().includes(lowerQuery) ||
  ...
  filteredAyahs.forEach((ayah, index) => {
    const card = document.createElement('div');
    card.className = 'ayah-card';

    // Number box with bangla digit
    ...
    card.innerHTML = `
      <div class="top-row">
        ...
        <div class="actions">
          <img src="play.png" class="play-icon" data-id="${ayah.ayahNumber}" title="প্লে" />
